...
void box_frontBack(int panjang = 1, int tinggi = 1, int p_x = 0, int p_y = 0, int p_z = 0, int geser_Z = 0) {
    ...
    glVertex3d((1 * panjang) + p_x, (1 * tinggi) + p_y, 0 + p_z + geser_Z);
    glVertex3d(0 + p_x, (1 * tinggi) + p_y, 0 + p_z + geser_Z);
    glEnd();
}
...
void box_sides(int lebar = 1, int tinggi = 1, int p_x = 0, int p_y = 0, int p_z = 0, int geser_X = 0) {
    ...
    glVertex3d(0 + p_x + geser_X, (1 * tinggi) + p_y, (-1 * lebar) + p_z);
    glVertex3d(0 + p_x + geser_X, (1 * tinggi) + p_y, 0 + p_z);
    glEnd();
}
...
void box_miringDP(int x_1 = -1, int x_2 = 1, int y_1 = -1, int y_2 = 1, int z_1 = -1, int z_2 = 1) {
    glBegin(GL_QUADS);
    glVertex3d(x_1, y_1, z_1);
    glVertex3d(x_2, y_1, z_1);
    glVertex3d(x_2, y_2, z_2);
    glVertex3d(x_1, y_2, z_2);
    glEnd();
}